...
public record BayRefDto(int BayID, string Name, bool IsActive);

/// <summary>ViewModel вкладки «Боксы» в разделе Справочники.</summary>
public sealed class BaysRefViewModel : BaseViewModel
{
    private BayRefDto? _selectedItem;
    ...
    public ObservableCollection<BayRefDto> Items { get; } = new();
    ...
    public async Task LoadAsync()
    {
        try
        {
            ...
            var list = await db.ServiceBays
                .Where(b => string.IsNullOrEmpty(search) || b.Name.Contains(search))
                .OrderBy(b => b.Name)
                .ToListAsync();
            Items.Clear();
            foreach (var b in list)
                Items.Add(new BayRefDto(b.BayID, b.Name, b.IsActive ?? false));
            StatusMessage = $"Боксов: {Items.Count}";
        }
    ...
    private async Task SaveAsync()
    {
        try
        {
            ...
            if (_editingId is null)
                db.ServiceBays.Add(new ServiceBay { Name = EditName.Trim(), IsActive = true });
            else
            {
                var e = await db.ServiceBays.FindAsync(_editingId.Value);
    ...
    private async Task ToggleActiveAsync()
    {
        ...
        {
            await using var db = new CarCareDbContext();
            var e = await db.ServiceBays.FindAsync(SelectedItem.BayID);
            if (e is not null) { e.IsActive = !(e.IsActive ?? false); await db.SaveChangesAsync(); }
            await LoadAsync();
        }
        catch (Exception ex) { StatusMessage = $"Ошибка: {ex.Message}"; }
    }
    ...
    private async Task DeleteAsync()
    {
        ...
        {
            await using var db = new CarCareDbContext();
            bool inUse = await db.OrderServices.AnyAsync(os => os.BayID == SelectedItem.BayID);
            if (inUse) { StatusMessage = $"Нельзя удалить «{SelectedItem.Name}» — используется в заказах."; return; }
            var e = await db.ServiceBays.FindAsync(SelectedItem.BayID);
            if (e is not null) { db.ServiceBays.Remove(e); await db.SaveChangesAsync(); }
            await LoadAsync();
        }
        catch (Exception ex) { StatusMessage = $"Ошибка: {ex.Message}"; }
    }
